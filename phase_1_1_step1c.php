...
$theme = query("SELECT * FROM theme_reason2 WHERE user = '$userid' ORDER BY pilih ASC");
...
<div class="w3-container w3-margin-top w3-large w3-mobile">
	...
	<div class="w3-row">
	  <div class="w3-col w3-container w3-xxlarge"><h1>原因:為什麼你修改了主題呢?</a></h1></div>
	</div>
	<div class="w3-row">
	  <div class="w3-col w3-container"><p>請選擇為什麼修改第一次內容的原因。</p></div>
	</div>
...
<?php foreach ( $theme as $row ) : ?>
		...
		<?php endif; ?>
		<!-- End div -->
	  <div class="w3-col w3-container w3-xxlarge w3-quarter"><h1>原因 <!-- <?= $i; ?> --> :</h1></div>
	  <div class="w3-col w3-container w3-half"><p>來源 : <?= $row["nama"]; ?></p><p>原因 : <?= $row["reason"]; ?></p></div>
	  <div class="w3-col w3-container w3-quarter"><a href="phase_1_1_step1c_edit.php?id=<?= $row["id"]; ?>" class="w3-button w3-section w3-green w3-ripple"><i class='fas fa-edit' style='font-size:24px'></i></a><a href="phase_1_1_step1c_delete.php?id=<?= $row["id"]; ?>" onclick="return confirm('Do you want to delete?');" class="w3-button w3-section w3-green w3-ripple"><i class='fas fa-trash-alt' style='font-size:24px'></i></a></div>
	</div>
...
<!-- End looping for the div table -->
	...
	<div class="w3-row">
		...
		<a href="phase_1_1_step1c_addreason.php" class="w3-button w3-section w3-green w3-ripple">Add reason (新增原因)</a><!-- <a href="phase_1_1_step1b_changeorder.php" class="w3-button w3-section w3-green w3-ripple">Change order</a> -->&nbsp<a href="phase_1_1_step1.php" class="w3-button w3-section w3-green w3-ripple"><i class='fas fa-undo'></i></a></div>
	</div> 
</div>
