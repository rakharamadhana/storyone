...
$group = query("SELECT * FROM group_diss WHERE user = '$userid' OR user2 = '$userid' OR user3 = '$userid' OR user4 = '$userid' OR user5 = '$userid' ");
$userku = query("SELECT id FROM user WHERE id = '$userid' ");
// $group = query("SELECT * FROM group_diss WHERE user = '$userid' ORDER BY pilih ASC");
...
<div class="w3-container w3-margin-top w3-large w3-mobile">
	...
	<div class="w3-row">
	  <div class="w3-col w3-container w3-xxlarge"><h1>階段三：個人和小組作品反思</a></h1></div>
	</div>
	<div class="w3-row">
	  <div class="w3-col w3-container"><p>此頁面你會看到專屬於你在平台上的號碼是〔x〕號。<br>

請點選「新增討論」，在「新增成員」區，加入組員的號碼後，你和小組成員即可在各自的帳號看到彼此寫得個人和小組作品的反思。
 <br>你的號碼是 <span style="font-size: 35px; color: yellow;">[ <?php foreach ( $userku as $row ) : ?>
			<?= $row["id"]; ?>
		<?php endforeach; ?> ]</span> 號.</p></div>
	</div>
...
<?php foreach ( $group as $row ) : ?>
		...
		<?php endif; ?>
		<!-- End div -->
	  <div class="w3-col w3-container w3-xxlarge w3-quarter"><h1><?= $row["group_name"]; ?><!-- <?= $i; ?> --> </h1></div>
	  <div class="w3-col w3-container w3-half"><p><?= $row["nama"]; ?></p></div>
	  <div class="w3-col w3-container w3-quarter"><a href="phase_3_3_edit_group.php?id=<?= $row["id"]; ?>" class="w3-button w3-section w3-green w3-ripple"><i class='far fa-edit' style='font-size:24px'></i></a><a href="phase_3_3_delete_group.php?id=<?= $row["id"]; ?>" onclick="return confirm('Do you want to delete?');" class="w3-button w3-section w3-green w3-ripple"><i class='far fa-trash-alt' style='font-size:24px'></i></a></div>
	</div>
...
<!-- End looping for the div table -->
	...
	<div class="w3-row">
		...
		<a href="phase_3_3_add_group.php" class="w3-button w3-section w3-green w3-ripple">Add discussion (新增討論)</a> <!-- <a href="phase_3_1_step1_changeorder_flipped.php" class="w3-button w3-section w3-green w3-ripple">Change order</a>--> &nbsp<a href="phase_3_1.php" class="w3-button w3-section w3-green w3-ripple"><i class='fas fa-undo'></i></a></div>
	</div> 
</div>
